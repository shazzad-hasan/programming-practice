...
def hangman(secretWord):
    ...
    * After each round, you should also display to the user the 
    ...
    while guess > 0:
        ...
        guessed_letters = lettersGuessed.copy()
        lettersGuessed.append(userInput)
        guess_word = getGuessedWord(secretWord, lettersGuessed)
            

        if userInput in guessed_letters:
            print("Oops! You've already guessed that letter: ", guess_word)
        elif userInput not in secretWord:
            print('Oops! That letter is not in my word: ', guess_word)
            guess -= 1
        else:
            print("Good guess: ", guess_word)
